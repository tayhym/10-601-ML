...
%         theta - (n+1) x 1 vector of predicted weights.
    ...
    %% BEGIN SOLUTION
    X_aug = [ones(size(X,1),1),X]; % augment each training example x with ones feature
    theta = minimize(@(theta_values) costLR(X_aug,y,theta_values,lambda),initial_theta);
%     theta = minimize(f, initial_theta);
    
    % check gradient - done,
    % checkGradient(@(x) costLR(X_aug, y, x, lambda), theta);
    
    %% END SOLUTION
    
end

% function [cost, grad, hess] = f(theta)
%     X_aug = [ones(size(X,1),1),X];
%     [cost, grad, hess] = costLR(X_aug,y,theta,lambda);
% end
